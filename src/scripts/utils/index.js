...
export const createModelProgressCallback = (onProgress) => {
  const fileProgress = {};

  return (progressData) => {
    if (!progressData) return;

    const { status, name, progress } = progressData;
    let message = "";

    if (status === "progress" && progress !== undefined && name) {
      const lowerName = name.toLowerCase();
      let label = "Model";
      if (lowerName.includes("encoder")) label = "Encoder";
      else if (lowerName.includes("decoder")) label = "Decoder";

      const rounded = Math.round(progress);
      const current = fileProgress[label] || 0;
      if (rounded >= current) {
        fileProgress[label] = rounded;
      }

      const parts = Object.entries(fileProgress)
        .map(([key, val]) => `${key}: ${val}%`)
        .join(" | ");
      
      if (parts.length > 0) {
        message = `Mengunduh model AI... ${parts}`;
      } else {
        message = "Menginisialisasi model AI...";
      }
      ...
      if (name) {
        ...
        delete fileProgress[label];
      }
      message = "Menginisialisasi model AI...";
    } else if (status === "ready") {
      message = "Model AI Siap";
    }

    if (message && onProgress) {
      onProgress({ message, progress: progressData.progress || 0 });
    }
  };
};

export const updatePerformanceStats = (stats, elapsed) => {
  stats.operations++;
